...
# Patch some debug code in the C++ wrapper. No dumping of scopes or memory.
cd "${coretbdir}" || exit
sed -i tb_top_verilator.cpp -e '/Verilated::scopesDump();/d'
sed -i tb_top_verilator.cpp -e '/dump_memory();/d'
...
# patch the BSP source
sed -i -e 's/^CFG_CFLAGS /#CFG_CFLAGS /' Makefile
...
# restore the source
cd "${bspdir}" > /dev/null 2>&1 || exit
git checkout handlers.S >> "${logf}" 2>&1
git checkout Makefile >> "${logf}" 2>&1

cd "${coresimdir}" > /dev/null 2>&1 || exit
git checkout Makefile >> "${logf}" 2>&1

cd "${coretbdir}" || exit
git checkout tb_top_verilator.cpp >> "${logf}" 2>&1
git checkout tb_top_verilator.sv >> "${logf}" 2>&1
